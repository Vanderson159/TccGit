<style>
@media screen and (max-width: 1920px){
    h1{
        text-align: center;
        margin-top: 1%;
    }

    .btnMan{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/man.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEstrada{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/estrada.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnPonto{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/pontoOnibus.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEmpresa{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/empresa.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }
    
    .div1{
        width: 210px;
        height: 142px;
        margin-left: 12%;
        margin-top: 5;
    }

    .div2{
        width: 210px;
        height: 142px;
        margin-left: 33%;
        margin-top: -140;
    }

    .div3{
        width: 210px;
        height: 142px;
        margin-left: 54%;
        margin-top: -140;
    }

    .div4{
        width: 210px;
        height: 142px;
        margin-left: 75%;
        margin-top: -140;
    }

    .div5{
        margin-left: -15px;
        margin-top: 60;
    }

    .btnMan:hover,.btnEstrada:hover, .btnPonto:hover, .btnEmpresa:hover{
       background-color: #ffcc00;
       background-image: none;
    }
    
    .label{
        font-family: Arial;
        font-size: 20;
        opacity: 0;
        color: black;
    }

    .label:hover{
        opacity: 100;
    }

    .Btnlogut{
        margin-top: -5%;
        margin-left: -32%;
        width: 120px;
        height: 50px;
        border: none;
        background-color: #BCB9B9;
        background-repeat: no-repeat;
        background-size: contain;
        background-position: center;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/logOut.png')?>');
    }

    .labelLogout{
        margin-top: 5%;
        margin-left: 45.5%;
        font-size: 20;
        font-weight: bold;
    }
}

@media screen and (max-width: 1600px){  
    h1{
        text-align: center;
        margin-top: 1%;
    }

    .btnMan{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/man.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEstrada{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/estrada.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnPonto{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/pontoOnibus.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEmpresa{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/empresa.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .div1{
        width: 210px;
        height: 142px;
        margin-left: 12%;
        margin-top: 5;
    }

    .div2{
        width: 210px;
        height: 142px;
        margin-left: 33%;
        margin-top: -140;
    }

    .div3{
        width: 210px;
        height: 142px;
        margin-left: 54%;
        margin-top: -140;
    }

    .div4{
        width: 210px;
        height: 142px;
        margin-left: 75%;
        margin-top: -140;
    }

    .div5{
        margin-left: -15px;
        margin-top: 60;
    }

    .btnMan:hover,.btnEstrada:hover, .btnPonto:hover, .btnEmpresa:hover{
       background-color: #ffcc00;
       background-image: none;
    } 

    .label{
        font-family: Arial;
        font-size: 20;
        opacity: 0;
        color: black;
    }

    .label:hover{
        opacity: 100;
    }

    .Btnlogut{
        margin-top: -5%;
        margin-left: -18%;
        width: 120px;
        height: 50px;
        border: none;
        background-color: #BCB9B9;
        background-repeat: no-repeat;
        background-size: contain;
        background-position: center;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/logOut.png')?>');
    }

    .labelLogout{
        margin-top: 2%;
        margin-left: 46%;
        font-size: 20;
        font-weight: bold;
    }
}

@media screen and (max-width: 1366px){
    h1{
        text-align: center;
        margin-top: 1%;
    }

    .btnMan{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/man.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEstrada{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/estrada.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnPonto{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/pontoOnibus.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }

    .btnEmpresa{
        background-repeat: no-repeat;
        text-align: center;
        width: 210px;
        height: 142px;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/empresa.png')?>');
        background-color: #282828;
        background-size: contain;
        background-position: center;
        border: 2px double #404040;
    }
    
    .div1{
        width: 210px;
        height: 142px;
        margin-left: 12%;
        margin-top: 5;
    }

    .div2{
        width: 210px;
        height: 142px;
        margin-left: 33%;
        margin-top: -140;
    }

    .div3{
        width: 210px;
        height: 142px;
        margin-left: 54%;
        margin-top: -140;
    }

    .div4{
        width: 210px;
        height: 142px;
        margin-left: 75%;
        margin-top: -140;
    }

    .div5{
        margin-left: -15px;
        margin-top: 25px;
    }

    .btnMan:hover,.btnEstrada:hover, .btnPonto:hover, .btnEmpresa:hover{
        background-color: #ffcc00;
        background-image: none;
    }
    
    .label{
        font-family: Arial;
        font-size: 20;
        opacity: 0;
        color: black;
    }

    .label:hover{
        opacity: 100;
    }

    .Btnlogut{
        margin-top: -5%;
        margin-left: -18%;
        width: 120px;
        height: 50px;
        border: none;
        background-color: #BCB9B9;
        background-repeat: no-repeat;
        background-size: contain;
        background-position: center;
        background-image: url('<?php echo base_url('assets/IMG/adminSite/logOut.png')?>');
    }
    
    .labelLogout{
        margin-top: 2%;
        margin-left: 46%;
        font-size: 20;
        font-weight: bold;
    }
}
</style>

<?php $session = session();?>
<?php if(isset($_SESSION['user'])) : ?>
    <?php 
        $login = $_SESSION['user'];
        $senha = $_SESSION['senha'];
    ?>
    <div class="retangulo"> 
        <h1>Área administrativa</h1>
        <div class="div5">
            <div class="div1"><a href="<?php echo base_url("public/admin/tabela")?>"><button class="btnMan"><p class="label">Administradores</p></a></button></div>
            <div class="div2"><a href="<?php echo base_url("public/admin/tabelaRuas")?>"><button class="btnEstrada"><p class="label">Ruas/Avenidas</p></a></button></div>
            <div class="div3"><a href="<?php echo base_url("public/admin/tabelaPontos")?>"><button class="btnPonto"><p class="label">Pontos</p></a></button></div>
            <div class="div4"><a href="<?php echo base_url("public/admin/tabelaEmpresa")?>"><button class="btnEmpresa"><p class="label">Empresas</p></button></a></div>
            <ul>
                <ol>
                    <label class="labelLogout">Logout:</label>
                </ol>
                <ol>
                    <form id="formLogin" class="form-signin text-center" method="post" action="<?php echo base_url("public/admin/logout")?>">
                        <button class="Btnlogut"></button>
                    </form>
                </ol>
            </ul>
        </div>
    </div>
<?php else : ?>
    <?php 
        echo view('header');
        echo view('login');
        echo view('footer');
    ?>
<?php endif ?>
